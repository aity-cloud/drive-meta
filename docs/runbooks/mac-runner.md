...
runner is a group runner of `aity-cloud/drive`, tagged `macos`, protected
refs only, tagged jobs only.
...
1. Create the runner in GitLab: group `aity-cloud/drive` -> Build ->
   Runners -> New group runner: tags `macos`, untick "Run untagged jobs",
   tick "Protected" (only protected branches/tags), lock to the group.
   Copy the `glrt-...` authentication token; it is shown once.
2. On the Mac:
